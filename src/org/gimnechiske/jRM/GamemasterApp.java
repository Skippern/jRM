/**
 * GamemasterApp: The main interface for the Gamemaster application
 * 
 * @author: Aun Johnsen
 * @file: GamemasterApp.java
 * 
 */
...
import org.gimnechiske.jRM.gui.GamemasterWindow;
...
public class GamemasterApp {
	...
	private static Preferences p = Preferences.userRoot().node("jRM")
			.node("GamemasterApp");
	...
	public static void loadPrefs() {
		...
		if (myGamemasterObject == null)
			myGamemasterObject = new Gamemaster();
		if (myGamemasterObject == null)
			System.out.println("[GM] ERROR: Still no gamemaster object!!!!");
	...
	public static void connectServer() throws Exception {
		...
		if (serverObj != null) {
			...
			showMessageDialog(
					null,
					"You are already connected, if this problem persists, try exiting the program and connect again.");
			...
			try {
				...
				System.out.println("[GM] ERROR: Cannot connect to "
						+ serverHost + "!");
				e.printStackTrace();
			}
		}
	...
	public synchronized static void registerGamemaster(Gamemaster gm)
			throws Exception {
		System.out.println("[GM] Creating Gamemaster object!");
		myGamemasterObject = gm;
		if (serverObj == null) {
			System.out
					.println("[GM] Trying to register Gamemaster without being connected to server");
		} else {
	...
	public static synchronized void getServerChatMessages() {
		...
		try {
			...
			// gmMessageQueue =
			// serverObj.getGamemaster(myGamemasterObject.getName()).getMessages();
			// gameMessageQueue =
			// serverObj.getGame(myGameObject.getName()).getMessages();
			System.out.println("[GM] " + gmMessageQueue.size()
					+ " messages in GM queue");
		} catch (Exception e) {
